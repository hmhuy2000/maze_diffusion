...
#     print(idx,'\t',new_image.mean().item(),new_image.max().item(),new_image.min().item(),new_image.std().item())
#     show_test_image(new_image.cpu())

# t = torch.Tensor([T-1]).type(torch.int64)
# plt.subplot(1, num_images+2, (T//stepsize) + 1)
# new_image, _ = forward_diffusion_sample(image, t,Xmin,Ymin,Xmax,Ymax,device)
# print(T-1,'\t',new_image.mean().item(),new_image.max().item(),new_image.min().item(),new_image.std().item())
# show_test_image(new_image.cpu())
...
def get_loss(model,prev, promt, image, t,Xmins,Ymins,Xmaxs,Ymaxs,render=False):
    ...
    if (render):
        wandb.log({
            ...
            "target":wandb.Image(reverse_transforms(image[0].cpu())),
            "x_0":wandb.Image(reverse_transforms(x_0[0].cpu())),
            "x_noisy":wandb.Image(reverse_transforms(x_noisy[0].cpu())),
            "noise_pred":wandb.Image(reverse_transforms(noise_pred[0].detach().cpu())),
                   })
    return F.l1_loss(noise, noise_pred)
    # return F.mse_loss(noise, noise_pred)

@torch.no_grad()
def sample_timestep(prev,promt,x, t,Xmin,Ymin,Xmax,Ymax):
    """
...
def sample_plot_image(epoch, prev,promt,image,Xmins,Ymins,Xmaxs,Ymaxs):
    ...
    for idx in range(5):
        ...
        print(f'{idx}, x_{T-1}',img.mean().item(),img.max().item(),img.min().item(),img.std().item())
        plt.figure(figsize=(120,20))
        plt.clf()
        num_images = 10
        stepsize = int(T/num_images)
        ...
        for i in range(0,T)[::-1]:
            ...
            img = sample_timestep(prev[idx].unsqueeze(0).cuda(),[promt[idx]],img, t,Xmin,Ymin,Xmax,Ymax)
            if i % stepsize == 0:
                show_tensor_image(None,promt[idx],(prev[idx]+img).detach().cpu(),t,num_images+2, i//stepsize+2)
